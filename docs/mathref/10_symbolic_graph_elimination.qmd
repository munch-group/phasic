...
*where $G(\boldsymbol{\theta})$ is the concrete graph obtained by evaluating all parameterized edges with $\boldsymbol{\theta}$, and $\operatorname{Eliminate}$ is the concrete graph elimination (Algorithm 11 in [06]).*
:::
...
*(a) Symbolic graph elimination (Algorithm 16) runs in $O(n^3)$ time and produces $O(n^2)$ symbolic edges.*

*(b) Instantiation (Algorithm 17) runs in $O(n^2 \cdot h)$ time per parameter vector, where $h$ is the maximum expression tree height. With CSE, $h$ is bounded by $O(n)$, giving $O(n^3)$ worst-case instantiation time, but in practice $h$ is much smaller.*
...
### Algorithm 16: Symbolic Graph Elimination
...
7:   SCC ← FindSCCs(G)                                 ▷ Algorithm 6 in [04]
8:   order ← TopologicalSort(SCC)                      ▷ Algorithm 7 in [04]
...
27:  end for
28:
29:  ▷ Phase 4: Elimination loop (mirrors Algorithm 11 in [06])
...
**Correctness.** Follows from @thm-10-symbolic-correctness. Each symbolic operation mirrors the corresponding concrete operation in Algorithm 11 in [06], and @thm-09-evaluation-correctness guarantees that evaluating the expression trees reproduces the numeric results.

### Algorithm 17: Symbolic Graph Instantiation
...
17:    inv_rate ← EvaluateExpression(rate_expr(i), θ)  ▷ Algorithm 15 in [09]
...
**Expression tree depth.** Without CSE, expression trees can grow exponentially with the number of elimination steps (each elimination can double the tree depth through multiplication). The intern table (@def-09-cse-interning) converts trees to DAGs, bounding the total number of unique nodes by $O(|V|^2)$. However, the evaluation algorithm (Algorithm 15 in [09]) traverses the tree structure, not the DAG, so shared subtrees may be evaluated multiple times. A memoized evaluation strategy would reduce this to $O(|V|^2)$ total operations per instantiation.
...
|-----------|------|----------|-------|
| Algorithm 16 (full) | `src/c/phasic_symbolic.c` | `ptd_graph_symbolic_elimination` | L248-L694 |
| Algorithm 16 (Phase 1: ordering) | `src/c/phasic_symbolic.c` | lines within `ptd_graph_symbolic_elimination` | Uses `ptd_find_strongly_connected_components` and `ptd_scc_graph_topological_sort` |
| Algorithm 16 (Phase 4: elimination) | `src/c/phasic_symbolic.c` | lines within `ptd_graph_symbolic_elimination` | L412-L563 |
| Algorithm 17 | `src/c/phasic_symbolic.c` | `ptd_graph_symbolic_instantiate` | L746-L805 |
| Algorithm 17 (batch) | `src/c/phasic_symbolic.c` | `ptd_graph_symbolic_instantiate_batch` | L810-L825 |
...
- **`ptd_graph_add_edge` in instantiation.** The instantiation function (Algorithm 17) calls `ptd_graph_add_edge` with a scalar weight (after converting probability to rate). This creates edges in constant mode, not parameterized mode, since the instantiated graph is concrete.
