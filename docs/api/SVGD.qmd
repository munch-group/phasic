...
:   Kernel bandwidth selection. Can be: - 'median_per_dim': Per-dimension median heuristic (default). Uses a   separate bandwidth for each parameter dimension, giving an anisotropic   kernel that adapts to different parameter scales. - 'median': Scalar median heuristic (isotropic kernel) - float: Fixed scalar bandwidth value - array_like: Fixed per-dimension bandwidth vector
...
<code>[**fixed**]{.parameter-name} [:]{.parameter-annotation-sep} [array_like or list of tuples]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>
...
| Name | Description |
| --- | --- |
| [analyze_trace](#phasic.SVGD.analyze_trace) | Analyze SVGD convergence and suggest parameter improvements. |
| [animate](#phasic.SVGD.animate) | Create an animation showing the evolution of a single parameter's distribution. |
| [animate_pairwise](#phasic.SVGD.animate_pairwise) | Create an animated pairwise scatter plot showing SVGD particle evolution. |
| [animate_parameter_pairs](#phasic.SVGD.animate_parameter_pairs) | Animate multiple parameter pairs simultaneously |
| [check_convergence](#phasic.SVGD.check_convergence) | Monitor convergence of SVGD by tracking statistics for n-dimensional parameters |
| [estimate_hdr](#phasic.SVGD.estimate_hdr) | Estimate the Highest Density Region (HDR) from particles. |
| [get_results](#phasic.SVGD.get_results) | Get inference results as a dictionary. |
| [map_estimate_from_particles](#phasic.SVGD.map_estimate_from_particles) | Find the MAP estimate from a set of particles by finding the particle |
| [map_estimate_with_optimization](#phasic.SVGD.map_estimate_with_optimization) | Refine MAP estimate by starting from the best particle and performing |
| [optimize](#phasic.SVGD.optimize) | Run SVGD inference with optional moment-based regularization. |
| [plot_ci](#phasic.SVGD.plot_ci) | Plot mean parameter trajectory with credible interval ribbon. |
| [plot_convergence](#phasic.SVGD.plot_convergence) | Plot convergence diagnostics showing mean and std over iterations. |
| [plot_hdr](#phasic.SVGD.plot_hdr) | Plot 2D highest-density region with optional marginal HPD bands. |
...
### analyze_trace { #phasic.SVGD.analyze_trace }

```python
phasic.SVGD.analyze_trace(burnin=None, verbose=True, return_dict=False)
```
...
### animate_pairwise { #phasic.SVGD.animate_pairwise }

```python
phasic.SVGD.animate_pairwise(
    true_theta=None,
    param_names=None,
    figsize=None,
    skip=0,
...
### animate_parameter_pairs { #phasic.SVGD.animate_parameter_pairs }

```python
phasic.SVGD.animate_parameter_pairs(
    param_pairs=None,
    true_params=None,
    figsize=(15, 5),
    save_as_gif=None,
...
### check_convergence { #phasic.SVGD.check_convergence }

```python
phasic.SVGD.check_convergence(every=1, text=None, param_indices=None)
```

Monitor convergence of SVGD by tracking statistics for n-dimensional parameters

### estimate_hdr { #phasic.SVGD.estimate_hdr }

```python
phasic.SVGD.estimate_hdr(alpha=0.95)
```
...
### map_estimate_from_particles { #phasic.SVGD.map_estimate_from_particles }

```python
phasic.SVGD.map_estimate_from_particles(unconstrained=False)
```
...
### map_estimate_with_optimization { #phasic.SVGD.map_estimate_with_optimization }

```python
phasic.SVGD.map_estimate_with_optimization(
    n_steps=70,
    step_size=0.01,
    unconstrained=False,
)
...
### optimize { #phasic.SVGD.optimize }

```python
phasic.SVGD.optimize(rewards=None, return_history=True)
```
...
<code>[**target**]{.parameter-name} [:]{.parameter-annotation-sep} [[array_like](`array_like`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>
...
<code>[]{.parameter-name} [:]{.parameter-annotation-sep} [([fig](`fig`), [ax](`ax`))]{.parameter-annotation}</code>
...
### plot_hdr { #phasic.SVGD.plot_hdr }

```python
phasic.SVGD.plot_hdr(
    alphas=[0.95, 0.5],
    idx=[0, 1],
    figsize=(5, 4),
    hexgrid=True,
...
```python
phasic.SVGD.summary(ci_method='hpd', ci_level=0.95)
```
