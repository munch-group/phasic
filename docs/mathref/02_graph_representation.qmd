...
*Algorithm 2 (Graph Validation) correctly identifies all parallel edges and runs in $O(|V| \cdot \Delta \log \Delta)$ time, where $\Delta = \max_{v \in V} |\mathcal{E}(v)|$ is the maximum out-degree.*
:::
...
*Algorithm 3 (Graph Clone) produces a graph $G'$ satisfying @def-02-clone in $O(|V| \cdot |E|)$ time in the current implementation, or $O(|V| + |E|)$ time with an index-based vertex map.*
:::
...
### Algorithm 1: AVL Find-or-Insert
...
|-----------|------|----------|-------|
| Algorithm 1 | `src/c/phasic.c` | `ptd_avl_find_or_create` | AVL tree find-or-insert |
| Algorithm 2 | `src/c/phasic.c` | `ptd_validate_graph` | Graph validation |
| Algorithm 3 | `src/c/phasic.c` | `ptd_clone_graph` | Graph deep clone |
| — | `src/c/phasic.c` | `ptd_graph_add_edge` | Edge insertion (uses @def-02-edge-list) |

**Deviations from pseudocode:**

- **Algorithm 1 (AVL):** The implementation stores parent pointers in each AVL node, which simplifies the rebalancing walk. The pseudocode abstracts this as `Rebalance(A, n_new)`.

- **Algorithm 2 (Validation):** The sort in the implementation uses `qsort` on the edge array, comparing target vertex pointers as integers. The pseudocode abstracts this as a generic comparison sort.

- **Algorithm 3 (Clone):** The pseudocode shows an index-based vertex map $\mu$. See the suspected code issue below regarding the actual implementation.
