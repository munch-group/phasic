...
class MCMC:
    ...
    **Mode 2: log_prob_fn** (for custom likelihoods)
    ...
    def __init__(
        ...
        # vmap requires model+data mode (no log_prob_fn).
        # likelihood_correction is allowed — it's applied outside vmap.
        can_vmap = (log_prob_fn is None and model is not None)
        if parallel is None:
    ...
    def run(self) -> MCMC:
        ...
        # JIT when _log_prob is traceable (model+data mode, no log_prob_fn).
        # likelihood_correction is applied outside JIT so doesn't block it.
        can_jit = (self.jit_enabled and self._log_prob_fn is None)
        if can_jit:
