...
```python
phasic.Graph(arg=None, ipv=None, **kwargs)
```
...
| Name | Description |
| --- | --- |
| [hierarchical](#phasic.Graph.hierarchical) | Whether this graph uses hierarchical trace-based computation. |
...
```python
phasic.Graph.accumulated_occupancy(*args, discrete=False, **kwargs)
```
...
:   If True, compute accumulated visits (discrete distribution). If False, compute accumulated visiting time (continuous distribution).
...
```python
phasic.Graph.cdf(time, discrete=False, **kwargs)
```
...
:   If True, compute CDF for discrete-time distribution (DPH). Requires that the graph was discretized via discretize().
...
:   If True, recompute trace even if a cached trace exists and is valid. Only applicable when Graph was created with hierarchical=True.

#### Returns {.doc-section .doc-section-returns}

<code>[]{.parameter-name} [:]{.parameter-annotation-sep} [[EliminationTrace](`EliminationTrace`)]{.parameter-annotation}</code>
...
>>> g = Graph(model, nr_samples=5, hierarchical=True)
>>> g.normalize()
...
```python
phasic.Graph.covariance(*args, discrete=False, **kwargs)
```
...
:   If True, compute discrete-time covariance (DPH distribution). Requires that the graph was discretized via discretize().
...
```python
phasic.Graph.discretize(rate, **kwargs)
```
...
```python
phasic.Graph.distribution_context(*args, discrete=False, **kwargs)
```
...
:   If True, create context for discrete-time distribution (DPH). Requires that the graph was discretized via discretize().
...
```python
phasic.Graph.expectation(rewards=[], discrete=False, **kwargs)
```
...
:   If True, compute discrete-time expectation (DPH distribution). Requires that the graph was discretized via discretize().
...
```python
phasic.Graph.pdf(time, discrete=False, **kwargs)
```
...
:   If True, compute PMF for discrete-time distribution (DPH). Requires that the graph was discretized via discretize().
...
```python
phasic.Graph.sample(n, discrete=False, **kwargs)
```
...
:   If True, sample from discrete-time distribution (DPH). Requires that the graph was discretized via discretize().
...
:   If True, compute for discrete-time distribution (DPH). Requires that the graph was discretized via discretize().
...
```python
phasic.Graph.stop_probability(time, discrete=False, **kwargs)
```
...
<code>[**bandwidth**]{.parameter-name} [:]{.parameter-annotation-sep} [[str](`str`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [\'median\']{.parameter-default}</code>

:   Kernel bandwidth selection method: - 'median': RBF kernel with median heuristic bandwidth (default) - 'rbf_adaptive': RBF kernel with adaptive bandwidth
...
:   Dimension of theta parameter vector. If None, inferred from the graph's parameterized edge structure via param_length(). Only required if theta_init is None and the graph has no parameterized edges.
...
```python
phasic.Graph.update_weights(theta, log=False, callback=None)
```
...
```python
phasic.Graph.variance(rewards=[], discrete=False, **kwargs)
```
...
:   If True, compute discrete-time variance (DPH distribution). Requires that the graph was discretized via discretize().
