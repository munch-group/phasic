...
(rewards runs going exact) ships WITH its opt-out. 2-D/multivariate
keeps its rejection (Batch G.2's design question).
"""
import numpy as np
import pytest
...
def test_rewards_2d_still_rejected_with_g2_message():
    g = _param_graph()
    rw = np.ones((2, g.vertices_length()))
    with pytest.raises(SvgdConfigError, match="2-D"):
