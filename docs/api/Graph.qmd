...
| [weight_formula](#phasic.Graph.weight_formula) | Per-edge weight formula string, evaluated in C (``weight_mode='formula'``). |
...
| [clear_from_cache](#phasic.Graph.clear_from_cache) | Delete this graph's entries from the on-disk caches. |
...
| [epoch_context](#phasic.Graph.epoch_context) | Build a single-epoch context for reading cumulative t-state probabilities. |
...
| [joint_prob_graph](#phasic.Graph.joint_prob_graph) | Build the joint-probability graph from this parameterized graph. |
| [joint_sojourn_graph](#phasic.Graph.joint_sojourn_graph) | Build the joint sojourn-read graph for granularity-free epoch reads. |
...
| [plot_scc_decomp](#phasic.Graph.plot_scc_decomp) | Visualise the SCC decomposition of this graph as a |
...
| [prewarm_cache](#phasic.Graph.prewarm_cache) | Populate the on-disk Stage A2 cache for this graph. If parameterized,  |
...
| [profile](#phasic.Graph.profile) | Profile this graph and recommend ``parallel_elimination``, |
| [pull_cache](#phasic.Graph.pull_cache) | Download the published compute artifact for this graph if available. |
| [push_cache](#phasic.Graph.push_cache) | Publish this graph's compute artifact to the phasic-traces registry. |
...
| [reward_visit_probability](#phasic.Graph.reward_visit_probability) | Probability of visiting any rewarded vertex before absorption. |
...
### clear_from_cache { #phasic.Graph.clear_from_cache }
...
<code>[**graph_cache**]{.parameter-name} [:]{.parameter-annotation-sep} [[bool](`bool`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [True]{.parameter-default}</code>
...
<code>[**parameterized_reward_compute**]{.parameter-name} [:]{.parameter-annotation-sep} [[bool](`bool`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [True]{.parameter-default}</code>
...
<code>[]{.parameter-name} [:]{.parameter-annotation-sep} [[dict](`dict`)\[[str](`str`), [int](`int`)\]]{.parameter-annotation}</code>
...
<code>[]{.parameter-name} [:]{.parameter-annotation-sep} [([jax](`phasic.jax`).[Array](`phasic.jax.Array`), [shape](`shape`)([n_t_vertices](`n_t_vertices`)))]{.parameter-annotation}</code>
...
### epoch_context { #phasic.Graph.epoch_context }
...
<code>[]{.parameter-name} [:]{.parameter-annotation-sep} [[EpochContext](`phasic.EpochContext`)]{.parameter-annotation}</code>
...
### joint_prob_graph { #phasic.Graph.joint_prob_graph }
...
### joint_sojourn_graph { #phasic.Graph.joint_sojourn_graph }

```python
phasic.Graph.joint_sojourn_graph()
```
...
:   New graph with attributes ``_joint_sojourn_graph = True``, ``_t_vertex_indices`` (new-graph indices of the original t-vertices), ``_ipv_target_indices`` / ``_ipv_target_states`` (the full IPV-edge layout; ``update_ipv`` expects a vector of length ``len(_ipv_target_indices)`` in this order), and the propagated ``_joint_prob_base_graph_indexer`` / ``_rewarded_props`` / ``_indexer``.
...
<code>[**observed_data**]{.parameter-name} [:]{.parameter-annotation-sep} [[array](`array`) or [SparseObservations](`SparseObservations`)]{.parameter-annotation}</code>
...
<code>[]{.parameter-name} [:]{.parameter-annotation-sep} [[MCMC](`MCMC`)]{.parameter-annotation}</code>
...
:   Number of model parameters (θ). When ``None`` it is resolved by weight mode, exactly as in :meth:`svgd`: from the graph's ``param_length()`` (linear/log), from a ``weight_formula``'s ``n_theta`` (formula mode), or — for a callback installed via the ``weight_callback`` property — it must be given explicitly (a callback cannot be introspected), otherwise this raises.
...
<code>[**label_fmt**]{.parameter-name} [:]{.parameter-annotation-sep} [[Callable](`collections.abc.Callable`)\[..., [str](`str`)\] \| None]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>

:   Callable for format node labels:

<code>[**rate_fmt**]{.parameter-name} [:]{.parameter-annotation-sep} [[Callable](`collections.abc.Callable`)\[[float](`float`)\] \| None]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>

:   Callable for format edge labels:

<code>[**color_by**]{.parameter-name} [:]{.parameter-annotation-sep} [[List](`typing.List`)\[[float](`float`)\] \| None]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>

:   List of values used for node fill colors.

<code>[**color_by_cmap**]{.parameter-name} [:]{.parameter-annotation-sep} [[List](`typing.List`)\[[float](`float`)\] \| None]{.parameter-annotation} [ = ]{.parameter-default-sep} [\'viridis\']{.parameter-default}</code>

:   List of values used for node fill colors.

<code>[**color_by_lim**]{.parameter-name} [:]{.parameter-annotation-sep} [[Tuple](`typing.Tuple`)\[[float](`float`)\] \| None]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>

:   Color map min, max limits for use with color_by. Default is min and max of values given by color_by.

<code>[**color_by_alpha**]{.parameter-name} [:]{.parameter-annotation-sep} [[float](`float`) \| None]{.parameter-annotation} [ = ]{.parameter-default-sep} [1.0]{.parameter-default}</code>
...
<code>[**taillabel**]{.parameter-name} [:]{.parameter-annotation-sep} [[bool](`bool`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [False]{.parameter-default}</code>
...
<code>[**graph_attr**]{.parameter-name} [:]{.parameter-annotation-sep} [[dict](`dict`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [{}]{.parameter-default}</code>

:   graphviz graph attributes to override defaults.

<code>[**node_attr**]{.parameter-name} [:]{.parameter-annotation-sep} [[dict](`dict`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [{}]{.parameter-default}</code>

:   graphviz node attributes to override defaults.

<code>[**edge_attr**]{.parameter-name} [:]{.parameter-annotation-sep} [[dict](`dict`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [{}]{.parameter-default}</code>
...
### plot_scc_decomp { #phasic.Graph.plot_scc_decomp }
...
<code>[**figsize**]{.parameter-name} [:]{.parameter-annotation-sep} [tuple of float]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>

:   Matplotlib figure size in inches. Ignored if ``ax`` is provided.

<code>[**cmap**]{.parameter-name} [:]{.parameter-annotation-sep} [[str](`str`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [\'viridis\']{.parameter-default}</code>

:   Matplotlib colormap name. Tiles are coloured by SCC index to make adjacent SCCs visually distinct.

<code>[**show_indices**]{.parameter-name} [:]{.parameter-annotation-sep} [[bool](`bool`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [True]{.parameter-default}</code>

:   Print the SCC index inside each tile when the tile is wide enough.

<code>[**annotate_sizes**]{.parameter-name} [:]{.parameter-annotation-sep} [[bool](`bool`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [True]{.parameter-default}</code>

:   Print the vertex count alongside the index.

<code>[**title**]{.parameter-name} [:]{.parameter-annotation-sep} [[bool](`bool`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [False]{.parameter-default}</code>
...
<code>[**ax**]{.parameter-name} [:]{.parameter-annotation-sep} [[matplotlib](`matplotlib`).[axes](`matplotlib.axes`).[Axes](`matplotlib.axes.Axes`) or None]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>
...
<code>[**fixed_mask**]{.parameter-name} [:]{.parameter-annotation-sep} [[jnp](`phasic.jnp`).[ndarray](`phasic.jnp.ndarray`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>
...
<code>[**observed_indices**]{.parameter-name} [:]{.parameter-annotation-sep} [array-like of int]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>
...
### prewarm_cache { #phasic.Graph.prewarm_cache }
...
### pull_cache { #phasic.Graph.pull_cache }
...
<code>[:]{.parameter-annotation-sep} [[phasic](`phasic`).[exceptions](`phasic.exceptions`).[PTDBackendError](`phasic.exceptions.PTDBackendError`)]{.parameter-annotation}</code>

:   On network failure or SHA-256 mismatch.

<code>[:]{.parameter-annotation-sep} [[phasic](`phasic`).[exceptions](`phasic.exceptions`).[PTDFormatError](`phasic.exceptions.PTDFormatError`)]{.parameter-annotation}</code>
...
### push_cache { #phasic.Graph.push_cache }
...
<code>[**domain**]{.parameter-name} [:]{.parameter-annotation-sep} [[str](`str`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>

:   Filtering keys stored in the entry's metadata.

<code>[**model_type**]{.parameter-name} [:]{.parameter-annotation-sep} [[str](`str`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>

:   Filtering keys stored in the entry's metadata.

<code>[**tags**]{.parameter-name} [:]{.parameter-annotation-sep} [[list](`list`)\[[str](`str`)\]]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>

:   Tags stored in the entry's metadata.

<code>[**license**]{.parameter-name} [:]{.parameter-annotation-sep} [[str](`str`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [\'MIT\']{.parameter-default}</code>

:   SPDX license identifier.

<code>[**author**]{.parameter-name} [:]{.parameter-annotation-sep} [[str](`str`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>

:   Override; default is ``"Name <email>"`` from ``git config user.{name,email}``.

<code>[**registry_repo**]{.parameter-name} [:]{.parameter-annotation-sep} [[str](`str`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [\'munch-group/phasic-traces\']{.parameter-default}</code>
...
<code>[**overwrite_branch**]{.parameter-name} [:]{.parameter-annotation-sep} [[bool](`bool`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [False]{.parameter-default}</code>
...
<code>[**validate_rewards**]{.parameter-name} [:]{.parameter-annotation-sep} [[bool](`bool`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [True]{.parameter-default}</code>
...
### reward_visit_probability { #phasic.Graph.reward_visit_probability }
...
<code>[**rewards**]{.parameter-name} [:]{.parameter-annotation-sep} [([array](`array`) - [like](`like`), [shape](`shape`)([n_vertices](`n_vertices`)))]{.parameter-annotation}</code>

:   Per-vertex reward; only the nonzero pattern is used.

<code>[**theta**]{.parameter-name} [:]{.parameter-annotation-sep} [[array](`array`) - [like](`like`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>
...
<code>[]{.parameter-name} [:]{.parameter-annotation-sep} [([float](`float`) or [jax](`phasic.jax`).[Array](`phasic.jax.Array`), [shape](`shape`)())]{.parameter-annotation}</code>
...
<code>[**n_particles**]{.parameter-name} [:]{.parameter-annotation-sep} [[int](`int`) or None]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>
...
:   Number of model parameters (θ). An explicit value here overrides any ``theta_dim`` set at construction (useful if the graph was modified, e.g. via ``extend()``). When not given, it is resolved by weight mode so it is always either specified or RELIABLY inferred:  - **linear / log** (default): inferred from the graph's   ``param_length()`` (the parameterized-edge coefficient length;   the two must match in these modes). - **formula** (``weight_formula`` set on the graph, or passed here):   inferred from the formula itself — the highest ``t`` index it uses   plus one (``n_theta``). A formula model therefore needs no   ``theta_dim`` at construction. An explicit ``theta_dim`` may exceed   ``n_theta`` (to reserve extra parameters); a smaller one raises. - **callback** (``callback`` passed here, or ``weight_callback`` set   on the graph): cannot be inferred — the callback receives the full   coefficient vector and may treat only some slots as parameters — so   you MUST pass ``theta_dim`` (or ``theta_init``), else this raises.  With ``epoch_starts`` (daisy chain) the per-epoch dimension comes from the graph and the optimised flat θ has length ``n_epochs × param_length``.
...
<code>[**fixed**]{.parameter-name} [:]{.parameter-annotation-sep} [list of (index, value) tuples or 1D array]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>
...
<code>[**tied**]{.parameter-name} [:]{.parameter-annotation-sep} [list of (local_idx, \[epoch_a, epoch_b, ...\]) tuples]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>
...
:   Parameter vector θ. Its required length depends on the weight mode (mirroring ``Graph(callback, theta_dim=...)``):  - **linear / log** (no ``callback`` / ``weight_formula``): length must   equal ``graph.param_length()`` (the edge coefficient length); a   mismatch raises. Set ``theta_dim`` (= ``param_length``) at   construction when the parameter count differs from the coefficient   length. - **callback / weight_formula**: θ may be SHORTER than the edge   coefficient length. The callback receives ``(theta, coefficients)``   and may use any subset; a formula references ``t0..`` (θ) and   ``c0..`` (the full per-edge coefficients). No length check is   imposed here — the callback/formula defines how θ maps to   coefficients.

<code>[**callback**]{.parameter-name} [:]{.parameter-annotation-sep} [[callable](`callable`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>

:   ``callback(theta, coefficients) -> weight`` for arbitrary per-edge weights (one-shot: applies for this call without changing the graph's persistent ``weight_mode``). Mutually exclusive with ``weight_formula``. Unlike linear/log it tolerates ``len(theta) != len(coefficients)``.
...
<code>[**weight_formula**]{.parameter-name} [:]{.parameter-annotation-sep} [[str](`str`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>

:   A weight formula string (see :attr:`weight_formula`). One-shot: this call's edge weights are computed from the formula in C, but the graph's persistent ``weight_mode`` is left unchanged (mirrors the ``callback`` argument). Useful for verifying a formula via ``graph.pdf(...)`` before running SVGD. Mutually exclusive with ``callback``.
