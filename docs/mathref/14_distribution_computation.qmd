...
**Source files:**
- `src/c/phasic.c` (functions: `_ptd_dph_probability_distribution_context_create`, `ptd_dph_probability_distribution_step`, `ptd_probability_distribution_context_create`, `ptd_probability_distribution_step`, `ptd_graph_pdf_with_gradient`)
- `api/c/phasic.h` (functions: `ptd_graph_pdf_with_gradient`, `ptd_graph_pdf_parameterized`)
...
<details>
<summary>Example</summary>
For a graph with starting vertex $v_0$, two transient vertices $v_1, v_2$, and one absorbing vertex $v_a$: $\mathbf{p}^{(0)} = (1, 0, 0, 0)$. After one step, $\mathbf{p}^{(1)}$ distributes mass from $v_0$ to its children according to the starting edge weights ($\boldsymbol{\alpha}$).
</details>
...
::: {.proof}
The key identity is the matrix exponential representation via uniformization [@jensen1953]:
...
| Parameterised PDF + gradient | `src/c/phasic.c` | `ptd_graph_pdf_with_gradient` | L10492--L10616 |
...
- **Parameterised PDF entry points.** For parameterised graphs the C API exposes two entries on top of the context-based forward algorithm above:
  - `ptd_graph_pdf_with_gradient(graph, t, g, params, n_params, &pdf, gradient)` (`src/c/phasic.c` L10492--L10616) takes an explicit parameter vector, computes the uniformisation rate $\lambda(\boldsymbol{\theta}) = \max_v \lambda_v(\boldsymbol{\theta})$ and its sensitivity $\partial\lambda/\partial\theta_j$ from the coefficients of the rate-attaining vertex, runs `compute_pmf_with_gradient` for $\lfloor gt \rfloor$ uniformised steps, and converts $\text{PMF} \to \text{PDF}$ via the product rule $\text{PDF}(t) = \text{PMF}(t) \cdot \lambda(\boldsymbol{\theta})$ with the matching gradient. Granularity is auto-selected to $\max(2\lambda_{\max}, 1000)$ as in the non-parameterised path; the gradient mode enforces a floor of 1000.
  - `ptd_graph_pdf_parameterized(graph, t, g, &pdf, gradient)` (`api/c/phasic.h` L1576--L1582) is the convenience wrapper that reads parameters from the graph's `current_params` slot (set by `ptd_graph_update_weight_parameterized`) and delegates to `ptd_graph_pdf_with_gradient`. The wrapper body is currently commented out in `src/c/phasic.c` (L10933+); callers should invoke `ptd_graph_pdf_with_gradient` directly until the wrapper is reinstated.
- **Relationship to the hierarchical elimination path of [13].** The forward algorithm formalised here operates on the *original* graph $G$, not on the eliminated form $G^*$ from [06] and not on the per-SCC parameterised reward-compute graphs (PRCs) of [13]. The hierarchical SCC composition is a moment-computation optimisation ([15]); the PDF/PMF code path is independent and is not affected by `PHASIC_HIERAR_ELIMINATION`. In a typical SVGD iteration the two are orthogonal: hierarchical composition accelerates the moment-based likelihood while the forward algorithm above computes the PDF on the same graph.
