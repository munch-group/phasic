...
| D1-E2 | Synthetic-SCC-graph guard (exact-gradient cores decline placeholder-coefficient graphs) | **merged** | `b3/d1-e2-synthetic-guard` (in-tree, deviation recorded) | `b3-d1-e2-guard-plan.md` v2 (2 plan refuters folded) | `b3-d1-derisk-findings.md` §E2 (+ dated correction) | G1 8/8; G2 targeted 407+1x, SCC row 407+3x, SLURM row 81+2x; G3 <see 11th stamp>; G4 2 refuters S-W-C — refuter A found the distributed serialize BYPASS (fixed, pinned), refuter B found 6 stale C decline-cause lists + G0 staleness (both fixed) | <merge hash> |
