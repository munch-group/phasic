...
### Continuous normalization {.unnumbered}
...
![Continuous graph normalization. Left: a phase-type graph with raw transition rates on edges (e.g., $A$ has outgoing rates $0.12$, $0.20$, $0.25$, $0.08$). Right: the normalized graph with transition probabilities in blue and vertex scalars $x_v = 1/\lambda_v$ in red (e.g., $x_A = 5$ since $\lambda_A = 0.20$). The starting vertex $S$ has scalar $0$ and the absorbing vertex $D$ has scalar $0$.](images/graph_normalization_example.pdf){#fig-05-normalization-example}
...
### Discrete normalization {.unnumbered}
...
::: {#alg-05-continuous-normalization}
...
|-----------|------|----------|-------|
| @alg-05-continuous-normalization | `src/c/phasic.c` | `ptd_normalize_graph` | L2337--L2360 |
| @alg-05-discrete-normalization | `src/c/phasic.c` | `ptd_dph_normalize_graph` | L2362--L2391 |
