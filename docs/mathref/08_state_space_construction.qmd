...
> **Example.** For the Kingman coalescent with $n$ lineages, the state is a single integer $\sigma = (n)$, and $\delta((n)) = \{((n-1), \binom{n}{2})\}$ for $n \geq 2$, and $\delta((1)) = \emptyset$ (absorbing). The rate $\binom{n}{2}$ is the coalescent rate when $n$ lineages remain.
...
*Description.* This algorithm implements the iterative state space construction (@def-08-state-space-construction). It performs a breadth-first traversal of the state space, using an AVL tree for $O(\log n)$ state deduplication. The algorithm corresponds to Algorithm 1 in Roikjer, Hobolth & Munch (2022).
