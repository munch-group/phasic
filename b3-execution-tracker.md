...
| H | Daisy final-epoch exact gradient | **de-risk DONE — GO; both queued user decisions taken 2026-08-13** (gate handling = additive C-side opt-out; joint-index `exact_grad` default stays False). Next: plan v3 + its own review, then implement | derisk/batchH-final-epoch | b3-batchH-plan.md v2 (v3 pending) | `b3-batchH-findings.md` (H0-H2 complete; composed grad 3.6e5× better than FD; default gate declines 100% of realistic calls yet accurate to 1e-13) | plan 2 refuters (S-W-C ×2, folded into v2) | — |
