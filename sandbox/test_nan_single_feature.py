"""Test NaN support with single-feature observations"""
import phasic
import numpy as np
...
print("=" * 70)
print("Testing NaN Support with Single-Feature Observations")
print("=" * 70)
...
for feature_idx in range(n_features):
    print(f"\n{'=' * 70}")
    print(f"Testing Feature {feature_idx} Only")
    print(f"{'=' * 70}")

    # Generate data for this feature only
    np.random.seed(42)
    ...
    print(f"Non-NaN values: {jnp.sum(~jnp.isnan(observed_data))} / {observed_data.size}")
    ...
    print(f"\nResults for Feature {feature_idx}:")
    ...
    if abs(final_theta[0] - true_theta[0]) < 2.0:  # Within 2 units
        print(f"  ✓ PASS: Converged to true value")
    else:
