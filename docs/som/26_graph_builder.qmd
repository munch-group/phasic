...
*where $f(t; G(\boldsymbol{\theta}))$ is the PDF computed by Algorithm 23 in [14] on the concrete graph.*
...
*where the moments are computed by Algorithm 25 in [15] on the concrete graph.*
...
**(2) PMF computation.** The `compute_pmf` method (lines 309--361) calls `build(theta)` to obtain $G(\boldsymbol{\theta})$, then calls `g.pdf(t_i, granularity)` for each time point (continuous case) or `g.dph_pmf(t_i)` for the discrete case. These are the same C++ methods that implement Algorithm 23 (CPH) and Algorithm 22 (DPH) from [14].

**(3) Moment computation.** The `compute_moments` method (lines 271--307) calls `build(theta)` and then `compute_moments_impl`, which iteratively calls `g.expected_waiting_time(rewards)` (the C++ implementation of Algorithm 25 from [15]) and applies the factorial scaling $\mathbb{E}[\tau^k] = k! \cdot r_0^{(k)}$ where $r_0^{(k)}$ is the $k$-th iterated expected waiting time at the starting vertex. $\square$
:::

## Algorithms

### Algorithm 40: GraphBuilder PMF Computation

*Description.* This algorithm describes the full pipeline from a serialized graph structure and parameter vector to PDF/PMF values. It is the C++ implementation behind the JAX FFI handler (Algorithm 39 in [25]). The key optimization is that JSON parsing and structure caching happen once (at `GraphBuilder` construction), while graph instantiation and forward algorithm execution happen per evaluation. For batch processing, the GraphBuilder is reused across all parameter vectors in the batch.
...
65:   for i ← 1 to n do
66:     if discrete then
67:       y_i ← G.dph_pmf(floor(t_i))        ▷ Algorithm 22 in [14]
68:     else
69:       y_i ← G.pdf(t_i, g)                ▷ Algorithm 23 in [14]
70:     end if
71:   end for
72:   return y
73: end function
...
80:   r ← G.expected_waiting_time(rewards)    ▷ Algorithm 25 in [15]
...
**Correctness.** Follows from @thm-26-equivalence. The `Build` function reconstructs $G(\boldsymbol{\theta})$ from the cached structure with weights evaluated at $\boldsymbol{\theta}$. The `ComputePMF` and `ComputeMoments` functions invoke the same C++ implementations used by Algorithms 22--23 ([14]) and Algorithm 25 ([15]).
...
|-----------|------|----------|-------|
| Algorithm 40 (constructor) | `src/cpp/parameterized/graph_builder.cpp` | `GraphBuilder::parse_structure` | L17--L116 |
| Algorithm 40 (weight) | `src/cpp/parameterized/graph_builder.cpp` | `GraphBuilder::compute_weight` | L196--L218 |
| Algorithm 40 (build) | `src/cpp/parameterized/graph_builder.cpp` | `GraphBuilder::build` | L118--L194 |
| Algorithm 40 (PMF) | `src/cpp/parameterized/graph_builder.cpp` | `GraphBuilder::compute_pmf` | L309--L361 |
| Algorithm 40 (moments) | `src/cpp/parameterized/graph_builder.cpp` | `GraphBuilder::compute_moments_impl` | L228--L269 |
| Algorithm 40 (PMF+moments) | `src/cpp/parameterized/graph_builder.cpp` | `GraphBuilder::compute_pmf_and_moments` | L363--L574 |
