...
*Algorithm 6 correctly identifies all strongly connected components of a phase-type graph $G = (V, E, W)$ in $O(|V| + |E|)$ time.*
:::
...
### Algorithm 6: Tarjan's SCC Detection
...
### Algorithm 7: Topological Sort of Condensation DAG
...
|-----------|------|----------|-------|
| Algorithm 6 (SCC core) | `src/c/phasic.c` | `strongconnect2` | L1830--L1895 |
| Algorithm 6 (main) | `src/c/phasic.c` | `ptd_find_strongly_connected_components` | L2143--L2313 |
| Algorithm 6 (acyclic fast path) | `src/c/phasic.c` | `ptd_find_strongly_connected_components_acyclic` | L2113--L2141 |
| Algorithm 6 (starting vertex isolation) | `src/c/phasic.c` | `ptd_isolate_starting_vertex_scc` | L1943--L2111 |
| Algorithm 7 | `src/c/phasic.c` | `ptd_directed_graph_topological_sort` | L3320--L3390 |
| Algorithm 7 (SCC wrapper) | `src/c/phasic.c` | `ptd_scc_graph_topological_sort` | L3406--L3408 |

**Deviations from pseudocode:**

- **Algorithm 6 (acyclic fast path):** The implementation first checks `ptd_graph_is_acyclic(graph)` (which itself attempts a topological sort of the original graph). If the graph is acyclic, it bypasses Tarjan's DFS entirely and constructs a trivial condensation where each vertex is its own SCC. This is an optimization: acyclic graphs are common in practice (e.g., Erlang distributions), and the trivial condensation avoids the overhead of DFS state allocation.

- **Algorithm 6 (memory over-allocation):** The arrays `scc_indices`, `low_links`, `scc_on_stack`, and `visited` are allocated with capacity `vertices_length * 10` instead of `vertices_length`. This appears to be a safety margin for graphs that grow during processing (e.g., when DPH normalization adds auxiliary vertices before SCC detection). However, the SCC algorithm does not modify the graph, so the factor of 10 is unnecessarily large.

- **Algorithm 6 (starting vertex isolation):** After SCC detection, `ptd_isolate_starting_vertex_scc` ensures the starting vertex $v_0$ is alone in SCC 0. If $v_0$ shares an SCC with other vertices, it is extracted into a new singleton SCC. This simplifies trace stitching ([13]) by guaranteeing a fixed structure for the starting SCC. The pseudocode abstracts this as a single call to `IsolateStartingVertex`.

- **Algorithm 6 (condensation edge deduplication):** The implementation uses an AVL tree (@def-02-avl-tree) to deduplicate target SCCs for each source SCC, then converts the tree to an array via DFS traversal and sorts the resulting edges by target SCC index. The pseudocode abstracts this as "add edge if not present."

- **Algorithm 6 (empty SCC filtering):** After Tarjan's algorithm, SCCs with zero internal vertices are discarded (lines 2182--2213). This handles a corner case where the algorithm produces empty component entries; in a correct execution this should not occur, but the implementation is defensive.

- **Algorithm 7 (struct casting):** The topological sort is implemented on the generic `ptd_directed_graph` struct. Both `ptd_graph` and `ptd_scc_graph` are cast to `ptd_directed_graph*` because their memory layouts are compatible in the first three fields (`vertices_length`, `vertices`, `starting_vertex`). Similarly, `ptd_scc_vertex` and `ptd_vertex` are cast to `ptd_directed_vertex`. This relies on C struct layout guarantees for the initial members.

- **Algorithm 7 (disconnected source handling):** The implementation includes a `has_pushed_all_others` flag that triggers a one-time scan for unreachable vertices with in-degree 0 when the queue first empties. This handles graphs with disconnected components (which can arise when absorbing vertices have no incoming edges from the main component).
