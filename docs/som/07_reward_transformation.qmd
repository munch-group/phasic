...
### Algorithm 13: Reward Transformation
...
6:   scc <- TarjanSCC(G)                               > Algorithm 6 in [04]
7:   order <- TopologicalSort(scc)                      > Algorithm 7 in [04]
...
43:      p_u_v <- w'(u -> v)
44:      > Sorted merge of u's and v's edge lists (same as Algorithm 10 in [06])
...
|-----------|------|----------|-------|
| Algorithm 13 (core) | `src/c/phasic.c` | `_ptd_graph_reward_transform` | L3550--L4023 |
| Algorithm 13 (CPH wrapper) | `src/c/phasic.c` | `ptd_graph_reward_transform` | L4025--L4036 |
| Algorithm 13 (DPH) | `src/c/phasic.c` | `ptd_graph_dph_reward_transform` | L4038--L4232 |
...
- **Edge list merge:** The same sorted-merge technique with sentinel dummy vertices is used as in Algorithm 10 (see Implementation Notes in [06]). The edge lists are sorted by pointer address, and existing edges to the same target are identified by pointer equality.
...
| $\mathbf{r}'$ | Effective reward vector (after threshold and normalization) | Algorithm 13 |
