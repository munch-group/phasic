...
**Moment computation via iterated Green matrix.** The $k$-th moment formula $\mathbb{E}[\tau^k] = k! \, \boldsymbol{\alpha} \mathbf{U}^k \mathbf{e}$ involves the $k$-th power of the Green matrix. In phasic, this is computed by $k$ applications of `expected_waiting_time` (Algorithm 25 in [15]), which applies graph elimination at each iteration. Each elimination costs $O(p^2)$ for dense graphs or $O(m)$ for sparse graphs, compared to $O(p^3)$ for computing $\mathbf{U}^k$ via matrix inversion. For $k$ moments of a sparse graph: phasic uses $O(k \cdot m)$ vs. $O(k \cdot p^3)$ for the matrix approach.
