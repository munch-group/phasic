...
def _compile_trace_library(cpp_code: str, trace_hash: str) -> str:
    ...
    lib_path = os.path.join(_secure_artifact_dir(), f"trace_log_lik_{trace_hash}.so")

    # Skip compilation if library already exists. Safe to trust: the artifact
    # dir is a per-user 0700 directory, so only we could have written this .so.
...
def clear_trace_cache() -> int:
    ...
    Removes all compiled shared libraries generated by
    trace_to_log_likelihood() with use_cpp=True from this process's
    secure artifact directory.
    ...
    """
    import glob
    pattern = os.path.join(_secure_artifact_dir(), "trace_log_lik_*.so")
