...
closed-folded into G.1. **Phases 1, 1b, 2, AND 3 are COMPLETE** (the moments adjoint is
exact across all four weight modes — linear cont+disc, log, formula,
callback-JAX-native — with 1-D rewards everywhere applicable). Next
up: Batch G.2 (2-D/multivariate leaf kwarg semantics; optional vehicle
for master §16b item 10); CC-2; deferred units. The core's contraction
switch has FIVE kinds (incl. the Batch-C binp exit).
...
| G.2 | Multivariate/2-D leaf `exact_moment_grad` + §16b item 10 | **plan-review** (USER DECISION 2026-08-15: FULL SYMMETRY — the kwarg honored on all moments leaves; wrapper gains it, svgd forwards it, R29's 2-D arm retired; + the loud ValueError replacing the 2-D-forward XLA crash; pure-Python batch, two plan refuters next; 9th-stamp environment caveat CLEARED by measurement) | b3/batchG2-multivariate (in-place; no C edits) | b3-batchG2-plan.md v1 |
