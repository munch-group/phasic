...
| [pdf](#phasic.Graph.pdf) | Compute probability density function using distribution context. |
| [plot](#phasic.Graph.plot) | Plots the graph using graphviz. See plot::plot_graph.py for more details. |
| [pmf](#phasic.Graph.pmf) | Compute probability mass function using forward algorithm. |
...
```python
phasic.Graph.covariance(*args, **kwargs)
```
...
```python
phasic.Graph.pdf(time, granularity=None, **kwargs)
```

Compute probability density function using distribution context.
...
<code>[**granularity**]{.parameter-name} [:]{.parameter-annotation-sep} [[int](`int`)]{.parameter-annotation} [ = ]{.parameter-default-sep} [None]{.parameter-default}</code>
...
:   PDF value(s) at the specified time point(s).

#### Notes {.doc-section .doc-section-notes}

This method PDF through uniformization as a (very good) approximation.
...
### pmf { #phasic.Graph.pmf }

```python
phasic.Graph.pmf(time, **kwargs)
```

Compute probability mass function using forward algorithm.
...
:   PMF value(s) at the specified time point(s).

#### Notes {.doc-section .doc-section-notes}

This method uses the forward algorithm (Algorithm 4) via uniformization
to compute the exact phase-type PMF, not an approximation.
