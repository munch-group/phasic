"""Tests for Graph.plot_scc_decomposition().
...
def test_plot_accepts_custom_axes():
    fig, ax = plt.subplots()
    g = build_toy_b()
    returned = g.plot_scc_decomposition(ax=ax)
...
def test_plot_accepts_custom_title():
    g = build_toy_b()
    ax = g.plot_scc_decomposition(title="my title")
...
def test_plot_show_indices_kwarg():
    ...
    g = build_toy_b()
    ax = g.plot_scc_decomposition(show_indices=False)
    assert ax is not None
    plt.close(ax.figure)
...
def test_plot_annotate_sizes_kwarg():
    ...
    g = build_toy_b()
    ax = g.plot_scc_decomposition(annotate_sizes=False)
