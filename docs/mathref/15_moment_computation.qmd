...
*Let $G = (V, E, W)$ represent $\operatorname{PH}_p(\boldsymbol{\alpha}, \mathbf{S})$ (@def-01-ph-graph) with elimination command sequence $C$ (@def-06-reward-compute-graph). The $k$-th factorial moment of the reward-transformed absorption time is computed by replaying $C$ with reward vector $\mathbf{r}$:*
...
*For a multivariate phase-type distribution $\mathbf{Y} \sim \operatorname{MPH}(\boldsymbol{\alpha}, \mathbf{S}, \mathbf{R})$ (@def-01-mph), the $j$-th marginal mean is $\mathbb{E}[Y_j] = \sum_v z_v \cdot R_{vj}$, and the second-order cross-moment between marginals $i$ and $j$ is*
